...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	if((e->env_tf.tf_eflags & FL_IOPL_3) == FL_IOPL_3) 
		e->env_tf.tf_eflags ^= FL_IOPL_3;
	if((e->env_tf.tf_eflags & FL_IOPL_2) == FL_IOPL_2) 
		e->env_tf.tf_eflags ^= FL_IOPL_2;
	if((e->env_tf.tf_eflags & FL_IOPL_1) == FL_IOPL_1) 
		e->env_tf.tf_eflags ^= FL_IOPL_1;
	return 0;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if((perm & PTE_U) != PTE_U || (perm & PTE_P) != PTE_P 
		||((perm | PTE_AVAIL | PTE_W) ^ PTE_AVAIL ^ PTE_W ^ PTE_U ^ PTE_P) != 0)
		return -E_INVAL;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if((uint32_t)srcva >= UTOP || (uint32_t)dstva >= UTOP) 
		return -E_INVAL;
	if(srcva != (void *)ROUNDUP((uint32_t)srcva, PGSIZE) ||
		dstva != (void *)ROUNDUP((uint32_t)dstva, PGSIZE))
		return -E_INVAL;
	struct PageInfo *p;
	pte_t * ppte;
	if((perm & PTE_U) != PTE_U || (perm & PTE_P) != PTE_P)
	...
	p = page_lookup(srce->env_pgdir, srcva, &ppte);
	if(p == NULL) 
		return -E_INVAL;

	if((perm & PTE_W) > 0 && (((*ppte) & PTE_W) == 0)) 
		return -E_INVAL;
	r = page_insert(dste->env_pgdir, p, dstva, perm);
	if(r < 0){
		page_free(p);
		return r;
	}
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	int r = envid2env(envid, &e, 1);
	if(r < 0) return r;
	if((uint32_t)va >= UTOP || va != (void *)ROUNDUP((uint32_t)va, PGSIZE)) 
		return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if(r < 0) return r;
	if(e->env_ipc_recving == 0 || e->env_ipc_from)
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
		...
		case SYS_page_alloc:
			...
			return r;
		case SYS_page_map:
			r = sys_page_map((envid_t)a1, (void *)a2, (envid_t)a3, (void *)a4, a5);
			return r;
		case SYS_page_unmap:
