...
mem_init(void)
{
	...
	boot_map_region(kern_pgdir, (uintptr_t)envs, ROUNDUP(NENV * sizeof(struct Env), PGSIZE), PADDR(envs), PTE_W | PTE_P);
	//////////////////////////////////////////////////////////////////////
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(*pde & PTE_P){
		...
		if(!create || (p = page_alloc(0)) == NULL)
			return NULL;
		pgtab = (pte_t *)page2kva(p);
		p->pp_ref = 1;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	a = (char*)ROUNDUP((uint32_t)va, PGSIZE);
	last = (char *)ROUNDUP((uint32_t)va + size - 1, PGSIZE);
	for(;;){
		if(a == last) break;
		if((pte = pgdir_walk(pgdir, (void *)a, 1)) == 0) return;
		*pte = pa | perm | PTE_P;
		a += PGSIZE;
		pa += PGSIZE;
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for(; a < last; a+=PGSIZE){
		if((uint32_t)a >= ULIM){
			user_mem_check_addr = (uintptr_t)(a < (char *)va?va:a);
			return -E_FAULT;
		}
		pte_t *pte = pgdir_walk(env->env_pgdir, a, 0);
		if(pte == NULL || (*pte & perm) != perm || (*pte & PTE_P) != PTE_P){
			user_mem_check_addr = (uintptr_t)(a < (char *)va?va:a);
			return -E_FAULT;
		}
	}
